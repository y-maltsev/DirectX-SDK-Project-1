...
void CModel::Update()
{
	...
	if(m_pAnimController && m_dwCurrentAnimation != -1){
		...
		if(m_dAnimationReverse)	m_pAnimController->AdvanceTime((Time-GetTickCount()) * 0.0006, NULL);
		else m_pAnimController->AdvanceTime((GetTickCount()-Time) * 0.001, NULL);
